...
function renderOptimizedRoute(points, baseLoc) {
    ...
    const SEGMENT_SIZE = 9; // 9 остановок на сегмент
    ...
    for (let i = 0; i < points.length; i += SEGMENT_SIZE) {
        ...
        // startPoint не используется для ссылки, но оставлен для совместимости
...
function createSegmentCard(container, points, segmentNum) {
    ...
    // Формируем URL в формате /dir/My+Location/addr1/addr2/.../addrN
    // Кодируем адреса: заменяем пробелы на + и применяем encodeURIComponent
    const encodedAddresses = points.map(p => 
        encodeURIComponent(p.raw.replace(/\s+/g, '+'))
    );
    // Добавляем "My+Location" как начало маршрута (текущее местоположение)
    const path = ['My+Location', ...encodedAddresses].join('/');
    const navUrl = `https://www.google.com/maps/dir/${path}`;
