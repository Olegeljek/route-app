...
function render(points) {
    ...
    for (let i = 0; i < points.length; i += size) {
        ...
        let drvUrl;
        if (isIOS) {
            // Специальный протокол для принудительного открытия приложения на iOS
            drvUrl = `comgooglemaps://?daddr=${destCoord}&directionsmode=driving&waypoints=${waypoints}`;
        } else {
            // Стандартный формат для Android и веба
            drvUrl = `https://www.google.com/maps/dir/?api=1&destination=${destCoord}&waypoints=${encodeURIComponent(waypoints)}&travelmode=driving&dir_action=navigate`;
        }
        ...
        box.innerHTML = `
            ...
            <div style="font-size:11px; margin-top:8px; color:#666;">🏁 Конец: ${chunk[chunk.length-1].label}</div>
        `;
