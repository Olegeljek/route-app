...
function buildSegmentGoogleMapsUrl(segmentPoints) {
  ...
  if (segmentPoints.length === 1) {
    const singlePoint = encodeURIComponent(segmentPoints[0].raw);
    return `https://www.google.com/maps/dir/?api=1&destination=${singlePoint}&travelmode=driving&dir_action=navigate`;
  }

  const origin = encodeURIComponent(segmentPoints[0].raw);
  const destination = encodeURIComponent(segmentPoints[segmentPoints.length - 1].raw);
  const waypoints = segmentPoints
    .slice(1, -1)
    .map((point) => encodeURIComponent(point.raw))
    .join("|");

  const waypointsParam = waypoints ? `&waypoints=${waypoints}` : "";
  return `https://www.google.com/maps/dir/?api=1&origin=${origin}&destination=${destination}${waypointsParam}&travelmode=driving&dir_action=navigate`;
}
